...
class GatewayController extends Controller
{
    ...
    public function checkPricingPlanAreaWise(Request $request, PathaoService $pathao)
    {
        //dd($request->all());
        $request->validate([
        ...
        // Response: { price, discount, promo_discount, plan_id, cod_percentage, final_price }
        return response()->json([
            'success'  => true,
            'pricePlan' => $response['data'] ?? $response,
        ], 200);
    }
    ...
    public function sendToPathao(Request $request, PathaoService $pathao)
    {
        ...
        $checkOrder           = Order::where('orderId', $request->orderId)->first();
        ...
        $pathaoOrder =  PathaoCourier::order()
            ->create([
                ...
                "recipient_name"      => $checkOrder->shipper_name, //"Gazi Giash Uddin", // Customer name
                "recipient_phone"     => $checkOrder->shipper_phone_number, //"01988846927", // Customer phone
                "recipient_address"   => $checkOrder->shipper_address, //"House 123, Mirpur-1, Dhaka", // Customer address
